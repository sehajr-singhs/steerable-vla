"""Build a self-contained Kaggle kernel with all source embedded.

The kernel decodes the embedded zip at runtime, so it needs NO dataset
mount. GPU is set via metadata 'enable_gpu': true.
"""
import base64, io, zipfile, os, json

SRC_DIR = os.path.join(os.path.dirname(__file__), "..", "src")
OUT_KERNEL = os.path.join(os.path.dirname(__file__), "..", "kaggle", "kernel.py")

# Build zip of all source
buf = io.BytesIO()
with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
    for root, dirs, files in os.walk(os.path.join(SRC_DIR, "steerable")):
        dirs[:] = [d for d in dirs if d != "__pycache__"]
        for f in files:
            if f.endswith(".py"):
                path = os.path.join(root, f)
                arc = os.path.relpath(path, os.path.dirname(SRC_DIR))
                zf.write(path, arc)
    # also include data_original.py (alias data.py)
    data_orig = os.path.join(SRC_DIR, "steerable", "data_original.py")
    if os.path.exists(data_orig):
        zf.write(data_orig, "src/steerable/data.py")
    # run_experiment.py
    run_exp = os.path.join(os.path.dirname(__file__), "run_experiment.py")
    if os.path.exists(run_exp):
        zf.write(run_exp, "scripts/run_experiment.py")

encoded = base64.b64encode(buf.getvalue()).decode()
print(f"Embedded {len(buf.getvalue())} bytes ({len(encoded)} b64 chars)")

KERNEL = f'''"""Steerable VLA — self-contained GPU study.

All source code is embedded below. No dataset mount needed.
Just attach GPU via Kaggle web UI and run.
"""
import base64, io, zipfile, os, sys, time, json, tempfile

# ── Extract embedded source ───────────────────────────────────────────
_EMBEDDED = "{encoded}"

_src_dir = os.path.join(tempfile.gettempdir(), "steerable_src")
os.makedirs(_src_dir, exist_ok=True)

data = base64.b64decode(_EMBEDDED)
with zipfile.ZipFile(io.BytesIO(data)) as zf:
    zf.extractall(_src_dir)

# Add to path
sys.path.insert(0, os.path.join(_src_dir, "src"))
sys.path.insert(0, _src_dir)
print(f"Extracted source to {{_src_dir}}", flush=True)
print(f"Files: {{len(os.listdir(_src_dir))}}", flush=True)
...
if torch.cuda.is_available():
    ...
    print(f"gpu memory: {{round(torch.cuda.get_device_properties(0).total_mem / 1e9, 1)}} GB", flush=True)
...
with open(OUT_KERNEL, "w") as f:
    f.write(KERNEL)
print(f"Kernel written to {OUT_KERNEL} ({len(KERNEL)} bytes)")

# Write metadata (no dataset_sources needed)
meta_path = os.path.join(os.path.dirname(OUT_KERNEL), "kernel-metadata.json")
meta = {
    "id": "sehajrsingh/steerable-vla-gpu-study",
    "title": "steerable-vla-gpu-study",
    "code_file": "kernel.py",
    "language": "python",
    "kernel_type": "script",
    "is_private": True,
