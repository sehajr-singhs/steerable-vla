...
\textbf{Expert ceiling.} The oracle succeeds on 100\% of train-family episodes
and 92.2\% of held-out zero-shot episodes (3 seeds, 30 episodes each).
...
\begin{table}[t]
\centering
\caption{Zero-shot held-out results (mean over 3 seeds, $n=30$ episodes per
variant-seed, GPU: NVIDIA T4, 200 epochs, 150 demonstrations). The
safety-filter finding (zero violations for our full system) is the study's
...
\textbf{Ours (full)} & 0.000 & 3.63 & 1.16 & \textbf{0.0} & 0.937 \\
\bottomrule
\end{tabular}
\end{table}
...
14--22 violations per episode. This is not a marginal improvement; the filter
provides a hard guarantee that the other variants cannot match. At miniature
scale the gripper workspace is a fixed box; the CBF--QP filter projects every
commanded action onto the admissible set with discrete forward invariance
(Proposition~\ref{prop:cbf}), and the environment counts violations instead of
silently clipping --- so violations are a faithful measure of filter absence.

All variants achieve high success ($\ge 0.87$) with similar intervention rates
($\sim 1.2$), indicating that the oracle assists the policy through the last
crossing regardless of architecture at this scale. Critically, on single-crossing
configs after the pipeline fixes described in Sec.~\ref{sec:disc}, the full
system achieves \textbf{100\% no-intervention success} (15/15 episodes, zero
violations), confirming that the flow expert \emph{can} learn the untangling
maneuver when expert demonstrations are reproducible and the evaluation
executes complete action chunks. The multi-crossing result shows that the
policy resolves $\sim 3.8$ of $\sim 4$ crossings on its own --- the subgoal
factorization composes for the first $3$ crossings; the final crossing requires
either more diverse training data or a more expressive subgoal predictor.
