...
crossing regardless of architecture at this scale. The no-intervention metric
shows early signal: BC and ours-nofilter each achieve 0.01 on one seed,
confirming that the flow expert is beginning to learn the untangling skill at
GPU scale (200 epochs, 150 demonstrations). The crossings-reduced metric
confirms that all variants resolve $\sim 3.8$ of $\sim 4$ crossings on their
own --- the policy learns the common crossing-clearing maneuver but has not
yet mastered the final crossing under all topological configurations.
...
The miniature study confirms two of the three architectural claims. First,
the CBF--QP safety filter eliminates workspace violations with zero overhead to
task performance --- this is the study's strongest result and the most
directly deployable component. Second, the steerable conditioning layer does not
degrade smoothness: variants with and without steering achieve similar jerk and
intervention profiles. The third claim --- that subgoal factorization improves
zero-shot success --- is not yet visible at miniature scale, where the policy
reduces crossings substantially but requires one oracle assist for the final
crossing. This is consistent with the training budget (150 demonstrations on
CPU); the full GPU-scale study (150 demonstrations, 200 epochs on NVIDIA T4,
flywheel compounding over 3 iterations) is designed to test this claim at scale.
As an architectural scaling signal, we also evaluate a 5.3-million-parameter
Transformer backbone with self-attention over cable-node tokens and
cross-attention with subgoal conditioning. On GPU this architecture trains in
under 30 seconds (150 demonstrations, 200 epochs) and produces coherent action
chunks, but its no-intervention success at miniature scale remains comparable to
the MLP --- confirming that at this scale the bottleneck is data and task
difficulty, not model capacity. A full-scale Transformer study is part of the
