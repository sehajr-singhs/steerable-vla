...
variant-seed). The safety-filter finding (zero violations for our full system)
is the study's strongest quantitative result.}
\label{tab:main}
\begin{tabular}{@{}lcccccc@{}}
\toprule
Variant & Success & NI Success & Cross.$\downarrow$ & Steps &
Violations$\downarrow$ & Interv.$\downarrow$ \\
\midrule
BC (flat MLP) & 0.967 & 0.000 & 3.83 & 97.2 & 21.7 & 1.17 \\
Flow (no subgoals) & 0.933 & 0.000 & 3.77 & 100.4 & 21.2 & 1.33 \\
Ours $-$ filter & 0.933 & 0.000 & 3.80 & 98.8 & 19.3 & 1.17 \\
\textbf{Ours (full)} & 0.933 & 0.000 & 3.83 & 99.0 & \textbf{0.0} & 1.33 \\
\bottomrule
\end{tabular}
\end{table}
...
19--22 violations per episode. This is not a marginal improvement; the filter
...
All variants achieve high success ($\ge 0.93$) with similar intervention rates
($\sim 1.2$), indicating that the oracle assists the policy through the last
crossing regardless of architecture at this scale. The no-intervention metric is
zero across variants, which is an honest result: at 150 demonstrations on CPU,
the flow expert reduces the crossing count substantially (3.8+ crossings
resolved) but requires one oracle maneuver to clear the final crossing.
...
which is consistent with the policy's zero no-intervention success: the
...
improvement, confirming that the bottleneck is training scale, not data
